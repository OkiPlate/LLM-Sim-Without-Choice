...
def continue1():
    ...
    if numM == 15 :
        reply = "Great. So, according to my calculations, the success rate for Option A is 81%" + " and for Option B is 63%"
        st.session_state["messages"].append({"role": "assistant", "content": reply})
        st.chat_message("assistant").write_stream(stream_data(reply))
    ...
    if numM == 16 :
        reply = "Based on the analysis and the data I have, we will go with Option A."
        st.session_state["messages"].append({"role": "assistant", "content": reply})
        st.chat_message("assistant").write_stream(stream_data(reply))
        numM = 18
